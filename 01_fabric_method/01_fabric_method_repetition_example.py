...
class Creator(ABC):
    ...
    def factory_method(self):
        """
        Обратите внимание, что Создатель может также обеспечать
    ...
    def some_operation(self) -> str:
        ...
        продуктов. Обычно он содержит некоторую базовую бизнес-логику,
        которая основана на объектах Продуктов, возвращаемых 
        фабричным методом. Подклассы могут косвенно изменять эту
        бизнес-логику, переопределяя фабричный метод и возвращая
        из него друго тип продукта
        """
        ...
        result = f"Creator: The same creator's code has just \
                            worked with {product.operation()}"
...
class ConcreteCreator1(Creator):
    ...
    тип абстрактного продукта, хотя фатически из метода возвращается
...
class Product(ABC):
    ...
    выполнять все конкретные продукты.
    """
...
class ConcreteProduct2(Product):
    def operation(self) -> str:
        return "Result pf the ConcreteProduct2"
